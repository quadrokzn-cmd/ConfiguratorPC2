...
from app.modules.auctions.match.matcher import (
    CandidateMatch,
    NomenclatureView,
    TenderItemView,
    serialize_rule_hits,
...
def derive_sku_ktru_codes(engine: Engine) -> int:
    """Заполняет nomenclature.ktru_codes_array для SKU с пустым массивом,
    ...
    """)
    update_sql = text(
        "UPDATE nomenclature SET ktru_codes_array = CAST(:codes AS TEXT[]) WHERE id = :id"
    )
    filled = 0
