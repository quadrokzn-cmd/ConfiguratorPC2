"""Импорт результатов обогащения от Claude Code в `nomenclature.attrs_jsonb`.

Поток:
    1) сканируем все JSON-файлы в `enrichment/done/`;
    ...
    5) обработанные файлы переносим в `enrichment/archive/<YYYY-MM-DD>/`.
"""
...
from app.modules.auctions.catalog.enrichment.exporter import ENRICHMENT_ROOT
...
def _process_file(path: Path, report: dict, *, dry_run: bool) -> None:
    ...
    try:
        with engine.begin() as conn:
            for item in payload["results"]:
                ...
                if dry_run:
                    exists = conn.execute(
                        text("SELECT 1 FROM nomenclature WHERE sku = :sku"),
                        {"sku": sku},
                    ).first()
                    if exists is None:
                        skus_unknown += 1
    ...
    if not dry_run:
        ...
        # формальный, но дешёвый: SKU уже в обороте, индекс по nomenclature_id
...
def import_done(dry_run: bool = False) -> dict:
    """Импортирует все JSON-файлы из enrichment/done/. Возвращает отчёт."""
...
def format_report(report: dict, *, dry_run: bool) -> str:
    lines: list[str] = []
    lines.append("=" * 72)
    title = "Импорт обогащения Claude Code: nomenclature.attrs_jsonb"
    if dry_run:
        title += "   [DRY-RUN]"
    lines.append(title)
    lines.append("=" * 72)
