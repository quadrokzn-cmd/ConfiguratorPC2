...
Файлы пишутся в `enrichment/pending/<brand>_<batch_id>.json`. Идемпотентность:
SKU, уже представленные в pending/ и done/, повторно не выгружаются. Архив
не учитывается — повторная выгрузка после архивации допустима (например, при
ручной правке).
"""
...
def export_manual_single(sku: str) -> Path:
    ...
    """
    engine = get_engine()
    sql = "SELECT sku, mpn, brand, name, category FROM nomenclature WHERE sku = :sku"
    with engine.connect() as conn:
