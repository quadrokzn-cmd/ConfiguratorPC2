...
После этапа 9Г.1: запускать обычно НЕ требуется
-------------------------------------------------
В этапе 9Г.1 эвристика is_likely_case_fan вынесена в
`shared/component_filters.py` и подключена в orchestrator прайсов.
Свежепоступившие в каталог корпусные вентиляторы автоматически создаются
с is_hidden=TRUE, так что разовая чистка больше не нужна.

Скрипт оставлен как ручной аварийный override на случай, если правило
детекции пропустит позицию (расширьте регэксп в component_filters.py
вместо того, чтобы запускать этот скрипт повторно).
