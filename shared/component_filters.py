...
#   • DDR-память (DDR3/DDR4/DDR5) и явные DIMM/UDIMM/SO-DIMM модули —
#     попадают в storages при ошибочной upstream-классификации (этап
#     11.6.2.7, реальные id Silicon Power / AGI / Digma DDR4 SO-DIMM);
#   • кулеры / вентиляторы для CPU — единичные кейсы вида Digma D-CPC95
#     (CPU-кулер в категории storages).
_NON_STORAGE_KEYWORDS = re.compile(
    ...
    # DDR-память: SO-DIMM ноутбучные модули и обычные DIMM ловятся как
    # явный stand-alone модуль ОЗУ. Защитный слой по форм-фактору
    # накопителя (NVMe/M.2/2280/mSATA) выше нас не пускает на реальные
    # SSD, в чьих именах изредка проскакивает «DDR» как часть тех.специф.
    r"\bDDR[345]\b|"
    r"оперативн\w+\s+памят\w+|"
    r"\b(?:SO[\s\-]?DIMM|UDIMM|DIMM)\b|"
    # CPU-кулер: явное слово «кулер»/«cooler» либо «вентилятор для CPU/
    # процессора». Учитываем только сильные сигналы, чтобы не задеть
    # legacy-модели SSD (в их raw_name «кулер» не встречается вовсе).
    r"\bкулер\b|\bcooler\b|"
    r"вентилятор\s+для\s+(?:CPU|процессор\w*)",
    flags=re.IGNORECASE,
)
