...
## 9. Корпусные вентиляторы в категории cooler (этап 9А.2.1)
...
**Текущее решение:** разовая массовая чистка скриптом
`scripts/hide_case_fans.py` — ищет позиции по эвристикам (regex по name +
отсутствие socket'ов и TDP) и помечает их `is_hidden=TRUE`.
См. итоговый отчёт `scripts/reports/case_fans_hidden_report.md` и SQL
для отката `scripts/reports/case_fans_backup_YYYYMMDD.sql`.

**Что нужно сделать на отдельном этапе (системное решение):**
добавить классификатор в `services/price_loaders/`, который при отсутствии
socket'ов и наличии regex-паттернов «AF/SP/PWM 120/140» помечает позицию
как корпусный вентилятор и пропускает в `is_hidden=True` автоматически —
чтобы новые корпусные вентиляторы не накапливались при следующих загрузках.
