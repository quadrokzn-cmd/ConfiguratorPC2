...
**После 2.5Б + backfill video_outputs + 2.5В (актуально на 2026-04-24,
после первого раунда агентов):**

| Поле | NULL | Всего |
|------|------|-------|
| gpu.tdp_watts        | 199 | 495 |
| gpu.needs_extra_power | 199 | 495 |
| gpu.video_outputs     | 198 | 495 |
| gpu.core_clock_mhz    | 225 | 495 |
| gpu.memory_clock_mhz  | 200 | 495 |

После завершения раунда 2.5В (GPU batches 2-5) — значения должны снизиться
ещё на ~80-100 позиций по каждому полю (оценка из 2.5Б: 120 полей за
40 позиций; × 3.5 = ~420 полей за 135 позиций).
...
**После 2.5В (текущий этап):**
- +31 поле video_outputs как `derived_from_name` (backfill).
- +2 новых домена в whitelist (afox-corp.com, gamerstorm.com).
- AI-агенты добавят ~200-300 полей (точные цифры после импорта,
  см. финальный отчёт этапа).

**Итоговое покрытие (ожидаемое после полного 2.5В):**

| Категория | Скелетов было | Закрыто | Осталось | % покрытия |
|---|---|---|---|---|
| psu         | 307 | 305 |   2 | 99.3% |
| storage     | 243 | 237 |   6 | 97.5% |
| case        | 839 | 775 |  64 | 92.4% (incl. supported_form_factors) |
| motherboard |   2 |   0* |   2 | 0% (AFOX mining; retry на 2.5В через afox-corp.com) |
| gpu         | 255 | ~60-90 | ~165-195 | 24-35% (vram_gb/type почти закрыты, системные поля — проблема) |
| cooler      | 561 | 425 | 136 | 75.8% (max_tdp_watts не публикуется на оф.сайтах) |

*После retry с afox-corp.com в 2.5В — ожидается 2/2 заполнено.
