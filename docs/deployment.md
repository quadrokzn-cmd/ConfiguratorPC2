...
### Этап 9В.3: Sentry для мониторинга ошибок

Технически опциональны (без них всё работает, но 5xx-ошибки никуда не
попадают). Подробности — [monitoring.md](monitoring.md).

| Переменная | Описание |
|---|---|
| `SENTRY_DSN_PORTAL` | DSN Sentry-проекта для портала. Прописывается **только** в Railway → portal. |
| `SENTRY_DSN_CONFIGURATOR` | DSN Sentry-проекта для конфигуратора. Прописывается **только** в Railway → configurator. |
| `SENTRY_DSN` | Fallback: если per-service переменной нет, оба сервиса возьмут эту. Удобно если оба сервиса должны лить в один проект. |
| `SENTRY_RELEASE` | Опциональная метка релиза (например, git-хеш). Без неё Sentry группирует события без `release`. |

Без любой из этих переменных init возвращает False, в логе одна
INFO-строка `"Sentry disabled (no SENTRY_DSN) for <service>"`.
