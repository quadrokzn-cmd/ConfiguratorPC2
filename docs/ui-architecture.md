...
этапов **UI-1**, **UI-2** и **UI-3** Пути B (слияние портала и
конфигуратора в одно FastAPI; полный план —
...
После UI-2 в конфигураторе стоят только 301-обработчики на место
переехавших страниц «Базы данных» (`app/main.py` около строки 160):
...
шесть обработчиков — простые `def`-функции без require_admin: 301
публичный, любой пользователь (в т.ч. с просроченной сессией) попадёт
на новый URL и уже там пройдёт обычный login-flow.
...
- **UI-4..UI-5 (план):** переезд самого Конфигуратора в
  `/configurator/*`, упразднение `config.quadro.tatar`. После UI-5
  `current_service='portal'` для всего; параметр и логика «другого
  сервиса» в `sidebar.html` упрощаются.
