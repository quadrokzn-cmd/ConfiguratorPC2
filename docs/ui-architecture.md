...
этапов **UI-1**, **UI-2**, **UI-3** и **UI-4** Пути B (слияние портала
и конфигуратора в одно FastAPI; полный план —
...
- **UI-4.5 (план):** перенос `app/services/auctions/` и `app/services/catalog/`
  из app/ в `shared/` или `portal/services/`. Сейчас они остаются в app/
  как legacy, а `portal/services/configurator/price_loaders/orchestrator.py`
  делает кросс-импорт `from app.services.auctions/catalog ...`. После UI-4.5
  кросс-импорт исчезнет.
