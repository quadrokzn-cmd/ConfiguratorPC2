...
## Раскладка приложения (общая для конфигуратора и портала)

С этапа 9Б.2.1 **оба сервиса** собираются на одном и том же скелете:

```
┌─────────┬────────────────────────────────────┐
│         │ [topbar — есть в конфигураторе,    │
│         │          нет в портале]            │
│ aside   ├────────────────────────────────────┤
│ sidebar │ main (контент страницы)            │
│ 248px   │                                    │
└─────────┴────────────────────────────────────┘
   ≤1024px → 72px (только иконки, текст скрыт)
```

CSS-классы каркаса (`@layer components` в `static/src/main.css`):
`kt-app-shell`, `kt-sidebar`, `kt-sidebar-header/nav/footer`, `kt-main`,
`kt-content`, `kt-topbar`. Их использует и `app/templates/base.html`,
и `portal/templates/base.html`.

Различия:

| Слот               | Конфигуратор                                   | Портал                                       |
|--------------------|------------------------------------------------|----------------------------------------------|
| Шапка сайдбара     | Лого UADRO + caption «КОНФИГУРАТОР»            | Лого UADRO + caption «ПОРТАЛ»                |
| Навигация          | Новый запрос, Проекты, История, админ-секция   | Главная, Пользователи (admin only)           |
| Подвал — back link | «← Портал» → `${PORTAL_URL}/`                  | «← Конфигуратор» → `${CONFIGURATOR_URL}/`    |
| Подвал — fx        | Курс ЦБ (общий партиал)                        | Курс ЦБ (общий партиал)                      |
| Подвал — user      | Карточка с logout (тот же DOM)                 | Карточка с logout (тот же DOM)               |
| Топбар             | Хлебные крошки + topbar_extra                  | Нет (навигация в сайдбаре)                   |

### Общий партиал курса ЦБ

`shared/templates/_partials/fx_widget.html` — единственное место, где
рендерится плашка курса ЦБ. Зависит от Jinja-global
`current_exchange_rate()`, которая зарегистрирована и в
`app/templating.py`, и в `portal/templating.py` (портал переиспользует
функцию из `app.templating` без копирования). Чтобы партиал был виден
обоим сервисам, в их `Jinja2Templates(directory=...)` передаётся список
из двух директорий: своей и `shared/templates/`.

### Блоки шаблона

В конфигураторе каждая страница может переопределить:
...
В портале — только `{% block content %}` и `{% block extra_head %}`
(топбара нет, страницы дашборда не требуют дополнительных слотов).
