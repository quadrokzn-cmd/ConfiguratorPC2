...
| `supplier_prices`        | (supplier_id × component) → цена USD, актуальность      |
| `unmapped_supplier_items`| Строки прайса, не сопоставленные автоматически          |
| `price_uploads`          | Журнал загрузок прайсов (когда, что, сколько строк)     |
...
по `name = supplier_name`. Email-адреса для трёх «новых»
(Netlab/Ресурс Медиа/Green Place) пока NULL — будут добавлены
руководителем отдельным апдейтом, как только пришлёт контакты.
