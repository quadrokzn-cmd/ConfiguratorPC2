...
| `component_field_sources` | Источники полей (regex/derived/claude_code/openai/manual) с подметкой `source_detail` (этап 11.6.1: `from_raw_name` для regex по supplier_prices.raw_name) |
...
## auto_price_loads / auto_price_load_runs (этап 12.3)

Каркас автозагрузок прайсов (миграция 028). Подробности — в
[auto_price_loads.md](auto_price_loads.md).

`auto_price_loads` — состояние подключения по каждому из 6 поставщиков
(одна строка на slug). Хранит: `enabled` (включён ли в ежедневный
обход APScheduler-а), `status` (`idle`/`running`/`success`/`error`),
timestamp'ы `last_run_at`/`last_success_at`/`last_error_at`,
`last_error_message` (truncate 2000 символов), FK
`last_price_upload_id` → `price_uploads(id) ON DELETE SET NULL`.

`auto_price_load_runs` — журнал запусков. Каждая попытка (manual или
scheduled) добавляет строку: `started_at`, `finished_at`, `status`
(`running`/`success`/`error`), `error_message` (truncate 2000),
`triggered_by` (`manual` / `scheduled`), FK на price_uploads.
Индекс `idx_auto_runs_slug_started(supplier_slug, started_at DESC)` —
для UI-журнала и быстрых выборок «последние N по поставщику».

Связь с `price_uploads`: каждый успешный запуск создаёт обычную
запись в `price_uploads` (статус `success`/`partial`/`failed`,
полный `report_json`), и FK обеих таблиц 12.3 указывают на её id.
Так UI «Журнал автозагрузок» сразу даёт ссылку «открыть отчёт» на
ту же страницу, что и ручные загрузки.
...
| 022 | `022_supplier_prices_raw_name.sql`                | `supplier_prices.raw_name TEXT` — оригинальное имя позиции от поставщика (Этап 11.4) |
| 023 | `023_component_field_sources_source_detail.sql`   | `component_field_sources.source_detail TEXT` — подметка regex-источника `from_raw_name` (Этап 11.6.1) |
| 028 | `028_auto_price_loads.sql`                        | `auto_price_loads` + `auto_price_load_runs` — каркас автозагрузки прайсов (Этап 12.3) |
