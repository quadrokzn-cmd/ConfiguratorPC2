...
from portal.services.configurator.auto_price.base import BaseAutoFetcher, register_fetcher
...
# Маппинг наших категорий в group_id (MaterialGroup) Resurs Media.
#
# История. На test-стенде (до 2026-05-12) маппинг был «Z383», «Z999-10001»
# и т.п. — 11 group_id с тестовыми идентификаторами. На боевом prod-стенде
# (подключён 2026-05-12) иерархия другая: все group_id вида «Z100XXX» /
# «Z101XXX», тест-группы отсутствуют. Из-за этого первый scheduled-тик
# 2026-05-13 07:40 МСК и ручной запуск 09:45 МСК получили «Resurs Media:
# GetPrices вернул пустой Material_Tab по всем 11 group_id» — fetcher
# спрашивал у API группы, которых в боевом каталоге нет.
#
# Этот маппинг — для prod-стенда. Источник: разведка по
# `resurs_media_catalog` (70 501 row, заполнен bootstrap'ом 2026-05-12),
# инспекция top-5 vendor + 5 случайных наименований каждой кандидатной
# группы. Все выбранные группы «чистые» (одна группа = одна наша
# категория). Группа `Z100808` (Synology mix: RAM-модули + сетевые карты +
# направляющие + HDD) намеренно исключена.
#
# Расклад по prod (rows округлены, актуально на 2026-05-13):
#   psu:         Z100173 (~426 ATX-БП от Aerocool/Chieftec/ZALMAN/FSP).
#   cooler:      Z100198 (~547 кулеры/вентиляторы/СВО, ID-Cooling и др.).
#   gpu:         Z100175-Z100178 (~37, разделены по вендору: ASUS/GIGABYTE/MSI/PALIT).
#   storage:     Z100160-Z100172 — SSD-семейство (~533); Z100109-Z100118 —
#                HDD-семейство (~114). Семейства склеены в одну категорию
#                по той же логике, что была на test-стенде: конфигуратор не
#                различает HDD/SSD на уровне `our_category`, только через
#                StorageRequirements.preferred_type на этапе подбора.
#   motherboard: Z100183 (ASUS) + Z100184 (GIGABYTE) + Z100185 (MSI), ~22.
#   ram:         Z100189 (DIMM), Z100195 (SO-DIMM), Z100193 (ECC Reg),
#                Z100891 (DDR5). SO-DIMM и ECC Reg в supplier_prices, но не
#                сматчатся с десктопным каталогом по mpn — осядут в
#                unmapped_supplier_items как unmatched, это штатно.
#   case:        Z100180 (~386 корпусов всех типоразмеров).
#   cpu:         Z100197 (~57 OEM-процессоров Intel/AMD).
_CATEGORY_GROUP_MAP: dict[str, list[str]] = {
    "psu":         ["Z100173"],
    "cooler":      ["Z100198"],
    "gpu":         ["Z100175", "Z100176", "Z100177", "Z100178"],
    "storage":     [
        "Z100160", "Z100167", "Z100168", "Z100169", "Z100170", "Z100171", "Z100172",
        "Z100109", "Z100112", "Z100114", "Z100117", "Z100118",
    ],
    "motherboard": ["Z100183", "Z100184", "Z100185"],
    "ram":         ["Z100189", "Z100195", "Z100193", "Z100891"],
    "case":        ["Z100180"],
    "cpu":         ["Z100197"],
}
