...
from portal.routers.configurator.main import _CATEGORY_ORDER, _prepare_variants
...
# Видимые границы как «Все границы» в Excel: thin + чёрный.
...
# Шапка проекта в строке 1 (с N1/O1 курса), строка 2 — пустая компактная
# (нужна, чтобы N2/O2 курса не съехали со своего места), заголовки
# таблицы — в строке 3, данные с 4-й.
_HEADER_ROW = 3
_DATA_START_ROW = 4
...
def build_project_xlsx(
    ...
    # Строка 2 содержит N2 «Курс ЦБ» и O2 — само число курса. Высоту
    # возвращаем к стандартной, чтобы текст «Курс ЦБ» не обрезался
    # сверху/снизу (в 8.6 высота была сжата до 8pt, что обрезало текст).
    ws["A2"] = None
    ...
    # --- Курс в N1/O1 (дата) и N2/O2 (значение) ---
    ws["N1"] = "Курс на"
    ws["N2"] = "Курс ЦБ"
