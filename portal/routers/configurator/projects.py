...
from portal.routers.configurator.main import (
    _CATEGORY_LABELS,
    _CATEGORY_ORDER,
    _prepare_variants,
)
from portal.services.configurator import budget_guard, spec_recalc, spec_service, web_service
from portal.services.configurator.nlu import process_query
from portal.services.configurator.web_result_view import enrich_variants_with_specs
...
# UI-4 (Путь B, 2026-05-11): префикс /configurator/ и scoped-проверка
# доступа к модулю «Конфигуратор ПК».
router = APIRouter(
    prefix="/configurator",
...
def projects_list(
    ...
    return templates.TemplateResponse(
        request,
        "configurator/projects_list.html",
        {
            "user":       user,
...
def projects_create(
    ...
    return RedirectResponse(url=f"/configurator/project/{pid}", status_code=status.HTTP_302_FOUND)
...
def project_detail(
    ...
    return templates.TemplateResponse(
        request,
        "configurator/project_detail.html",
        {
            "user":            user,
            "csrf_token":      get_csrf_token(request),
            "project":         project,
...
def project_rename(
    ...
    return RedirectResponse(url=f"/configurator/project/{project_id}", status_code=status.HTTP_302_FOUND)
...
def project_delete(
    ...
    return RedirectResponse(url="/configurator/projects", status_code=status.HTTP_302_FOUND)
...
def project_new_query_form(
    ...
    return templates.TemplateResponse(
        request,
        "configurator/project_new_query.html",
        {
            "user":       user,
            "csrf_token": get_csrf_token(request),
            "project":    project,
...
def project_new_query_submit(
    ...
    if not raw_clean:
        ...
        return RedirectResponse(
            url=f"/configurator/project/{project_id}/new_query",
    ...
    if budget.is_blocked:
        ...
        return RedirectResponse(
            url=f"/configurator/project/{project_id}?highlight={qid}",
            status_code=status.HTTP_302_FOUND,
        )
