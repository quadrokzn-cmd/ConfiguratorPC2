...
# UI-4 (Путь B, 2026-05-11): префикс /configurator/ для NLU-формы и
# истории. dependencies=[Depends(require_configurator_access)] заменяет
# глобальную middleware _enforce_configurator_permission из app/main.py —
# проверка permissions['configurator'] теперь scoped по этим трём роутерам.
router = APIRouter(
    prefix="/configurator",
...
def query_submit(
    ...
    if not raw_clean:
        ...
        return RedirectResponse(url="/configurator/", status_code=status.HTTP_302_FOUND)
