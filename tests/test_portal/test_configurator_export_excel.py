...
from portal.services.configurator.export import excel_builder
...
def _mock_cbr(rate: str = "92.5000", d: date | None = None):
    ...
    return patch(
        "portal.routers.configurator.export.exchange_rate.get_usd_rate",
...
def test_excel_endpoint_returns_xlsx(
    ...
    with _mock_cbr():
        r = manager_client.get(f"/configurator/project/{pid}/export/excel")
...
def test_excel_endpoint_forbidden_for_other_user(
    ...
    with _mock_cbr():
        r = app_client.get(f"/configurator/project/{pid}/export/excel")
    assert r.status_code == 403


def test_excel_endpoint_404_for_missing_project(manager_client):
    with _mock_cbr():
        r = manager_client.get("/configurator/project/999999/export/excel")
