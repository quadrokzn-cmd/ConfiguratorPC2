...
def _get_csrf(client: TestClient, url: str = "/configurator/projects") -> str:
    r = client.get(url)
...
def _create_project(client: TestClient) -> int:
    ...
    token = _get_csrf(client, "/configurator/projects")
    r = client.post("/configurator/projects", data={"csrf_token": token})
    assert r.status_code == 302
    loc = r.headers["location"]
    assert loc.startswith("/configurator/project/")
...
def both_variants_response(monkeypatch):
    ...
    from portal.routers.configurator import main as main_router, projects as project_router
...
def test_projects_list_shows_only_mine_for_manager(
    ...
    with TestClient(app, follow_redirects=False) as c2:
        ...
        r = c2.get("/configurator/projects")
        assert r.status_code == 200
...
def test_projects_list_shows_all_for_admin(
    ...
    r = app_client.get("/configurator/projects")
    assert r.status_code == 200
...
def test_submit_from_index_redirects_to_project_with_highlight(
    ...
    r = manager_client.get(f"/configurator/project/{pid}?highlight={qid}")
    assert r.status_code == 200
...
def test_select_variant_creates_spec_item(
    ...
    r = _ajax(manager_client, f"/configurator/project/{pid}/select",
...
def test_deselect_variant_removes_item(manager_client, mock_process_query):
    pid = _create_project(manager_client)
    qid = _submit_query_to(manager_client, pid)
    csrf = extract_csrf(manager_client.get(f"/configurator/project/{pid}").text)
    _ajax(manager_client, f"/configurator/project/{pid}/select",
          {"query_id": qid, "variant_manufacturer": "Intel", "quantity": 2},
          csrf=csrf)

    r = _ajax(manager_client, f"/configurator/project/{pid}/deselect",
...
def test_update_quantity_recalcs_totals(manager_client, mock_process_query):
    ...
    _ajax(manager_client, f"/configurator/project/{pid}/select",
          {"query_id": qid, "variant_manufacturer": "Intel", "quantity": 1},
          csrf=csrf)
    r = _ajax(manager_client, f"/configurator/project/{pid}/update_quantity",
...
def test_foreign_manager_cannot_access_project(
    ...
    r = app_client.get(f"/configurator/project/{pid}")
...
def test_rename_project_changes_name(manager_client, db_session):
    ...
    csrf = extract_csrf(manager_client.get(f"/configurator/project/{pid}").text)
    r = manager_client.post(
        f"/configurator/project/{pid}/rename",
...
def test_delete_project_removes_spec_items(
    ...
    r = manager_client.post(
        f"/configurator/project/{pid}/delete",
        data={"csrf_token": csrf},
    )
    assert r.status_code == 302
    assert r.headers["location"] == "/configurator/projects"
...
def test_delete_query_removes_its_spec_items(
    ...
    r = manager_client.post(
        f"/configurator/project/{pid}/query/{qid}/delete",
        data={"csrf_token": csrf},
    )
    assert r.status_code == 302
...
def test_csrf_required_on_ajax_select(manager_client, mock_process_query):
    ...
    r = manager_client.post(
        f"/configurator/project/{pid}/select",
...
def test_csrf_required_on_ajax_deselect(manager_client, mock_process_query):
    ...
    qid = _submit_query_to(manager_client, pid)
    r = manager_client.post(
        f"/configurator/project/{pid}/deselect",
...
def test_specification_panel_no_variant_label_under_name(
    ...
    html = manager_client.get(f"/configurator/project/{pid}").text
