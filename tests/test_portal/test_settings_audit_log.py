# Тесты /settings/audit-log и интеграции write_audit
# (бывший /admin/audit; перенос на этапе UI-3 Пути B, 2026-05-11).
#
# Покрывает:
...
def test_audit_page_requires_admin_anon(portal_client):
    """Аноним → 302 на /login."""
    r = portal_client.get("/settings/audit-log")
...
def test_audit_page_forbids_manager(manager_portal_client):
    """Менеджер → 403."""
    r = manager_portal_client.get("/settings/audit-log")
...
def test_audit_page_filters_by_user(
    ...
    r = admin_portal_client.get(
        f"/settings/audit-log?user_id={manager_user['id']}"
    )
    assert r.status_code == 200
...
def test_audit_page_filters_by_action(admin_portal_client, db_session):
    ...
    r = admin_portal_client.get("/settings/audit-log?action=user.create")
    assert r.status_code == 200
...
def test_audit_page_filters_by_service(admin_portal_client, db_session):
    ...
    r = admin_portal_client.get("/settings/audit-log?service=configurator")
    assert "Найдено записей: 1" in r.text
...
def test_audit_page_filters_by_date_range(admin_portal_client, db_session):
    ...
    записи audit.view, которые порождает сам GET /settings/audit-log."""
    db_session.execute(
        _t(
    ...
    r = admin_portal_client.get(
        "/settings/audit-log?action=date.test"
...
def test_audit_page_paginates(admin_portal_client, db_session):
    ...
    r = admin_portal_client.get("/settings/audit-log?action=act.*")
    assert r.status_code == 200
...
def test_audit_csv_export_returns_csv(admin_portal_client, db_session):
    ...
    r = admin_portal_client.get("/settings/audit-log/export")
    assert r.status_code == 200
...
def test_audit_view_writes_self_audit_record(admin_portal_client, db_session):
    """Открываем /settings/audit-log и проверяем, что в логе появляется
...
def test_audit_page_handles_empty_user_id_filter(admin_portal_client):
    ...
    r = admin_portal_client.get("/settings/audit-log?user_id=&action=")
    assert r.status_code == 200


def test_audit_page_handles_empty_date_filters(admin_portal_client):
    r = admin_portal_client.get("/settings/audit-log?date_from=&date_to=")
    assert r.status_code == 200


def test_audit_page_handles_empty_page(admin_portal_client):
    """Пустой page= → дефолт 1, без 422."""
    r = admin_portal_client.get("/settings/audit-log?page=")
    assert r.status_code == 200
...
def test_audit_page_handles_invalid_user_id_gracefully(admin_portal_client):
    ...
    r = admin_portal_client.get("/settings/audit-log?user_id=abc")
    assert r.status_code == 200
...
def test_audit_view_payload_excludes_empty_filters(
    ...
    r = admin_portal_client.get("/settings/audit-log?action=&user_id=&date_from=")
    assert r.status_code == 200
...
def test_backup_manual_run_writes_audit(admin_portal_client, db_session, monkeypatch):
    ...
    from portal.routers.settings import backups as settings_backups
    monkeypatch.setattr(
        settings_backups, "_run_backup_safely", lambda: None,
    )

    # Открываем главную, чтобы достать csrf_token (на /settings/backups
