...
def _inner_tbl(doc):
    """КП-таблица (этап 9А.2.7: внешняя обёрточная таблица убрана,
    таблица позиций — единственная в body)."""
    return doc.tables[0]._tbl


def _outer_tbl(doc):
    # Этап 9А.2.7: внешней таблицы больше нет — возвращаем ту же таблицу
    # позиций для совместимости с тестами на запасы по ширине.
    return doc.tables[0]._tbl
