...
### UI-5 — финальная зачистка [x]

Удалить:
- `app/main.py`, `Dockerfile`, `railway.json`, `app/templates/` (после переноса)
- Railway-сервисы `configurator` (prod) и `configurator-preprod` (pre-prod) — **вне scope Claude Code**, удаляются собственником через Railway UI
- DNS-записи `config.quadro.tatar` и `config-preprod.quadro.tatar` (через Reg.ru) — удаляются собственником
...
**Решение собственника 2026-05-11:** Вариант A (полное удаление, не сохранять 301-редиректы со старого хоста). URL admin-страниц `/admin`, `/admin/budget`, `/admin/queries` сохранены без редиректов — менеджеров с закладками нет.

**Артефакты UI-5 (2026-05-11):**

- `shared/config.py` — новый файл: `Settings` dataclass (полная копия из бывшего `app/config.py`, минус dead-field `run_scheduler`/`RUN_SCHEDULER` после UI-4.5). Единый источник `settings` для всего репо.
- `shared/db.py` — обновлён: `from app.config import settings` → `from shared.config import settings`. Это устранило последний `app/ ← shared/` импорт.
- `portal/templating.py` — расширен: туда переехал живой код `to_rub`/`fmt_rub`/`current_exchange_rate` из бывшего `app/templating.py`. `_safe_get_current_rate()` использует ленивые импорты `shared.db.SessionLocal` и `portal.services.configurator.export.exchange_rate`, чтобы не тянуть тяжёлые зависимости на старте.
- `portal/routers/admin.py` — новый файл: 3 admin-страницы `/admin` (dashboard), `/admin/budget`, `/admin/queries`. Депенденси `require_admin` из `shared.auth`. Подключён в `portal/main.py` ДО точечных catch-all 301 `/admin/users`, `/admin/backups`, `/admin/audit` (которые задают редиректы на `/settings/*` ещё с UI-3).
- `portal/templates/admin/{dashboard,budget,all_queries}.html` — `git mv` из `app/templates/admin/`. В `dashboard.html` поправлены ссылки `/admin/mapping` → `/databases/mapping` (UI-2 правка) и `/query/{id}` → `/configurator/query/{id}` (UI-4 правка). В `all_queries.html` — `/query/{id}` → `/configurator/query/{id}` (две точки замены: `data-href` атрибут и `<a href>`).
- `portal/templates/export/{kp_template.docx,project_template.xlsx}` — `git mv` из `app/templates/export/`. Параллельно в `portal/services/configurator/export/{excel_builder,kp_builder}.py` обновлён `_TEMPLATE_PATH`: `parents[4] / "app" / "templates" / "export" / X` → `parents[3] / "templates" / "export" / X` (от файла `portal/services/configurator/export/X.py` parents[3] = `portal/`).
- `portal/templates/base.html` — добавлена URL-ветка для admin-страниц: `/admin` (точно), `/admin/budget*`, `/admin/queries*` → `active_section='configurator'`, `active_subsection=none`. Подсветка sidebar на admin-дашборде показывает «Конфигуратор ПК» развёрнутым.
- `portal/main.py` — `from app.config import settings` → `from shared.config import settings`. Подключён `admin.router`. Импорт `admin` добавлен в группу `from portal.routers import (…)`.
- Массовая замена импортов через regex-скрипт (54 файла, 4 паттерна одним проходом):
  - `from app.config import …` → `from shared.config import …` (13 файлов: portal/, scripts/, shared/db.py)
  - `from app.database import …` → `from shared.db import …` (20 файлов)
  - `from app.auth import …` → `from shared.auth import …` (3+ файла)
  - `from app.templating import …` → `from portal.templating import …` (1 файл — portal/templating.py)
- `git rm -rf app/` — полная папка целиком (17 файлов), включая `app/main.py`, `app/auth.py`, `app/config.py`, `app/database.py`, `app/templating.py`, `app/routers/admin_router.py`, `app/templates/admin/`, `app/templates/_macros/`, `app/templates/base.html`, `app/models/`, `app/schemas/`, `app/services/__init__.py`. После этого `grep -rn 'from app\.' --include='*.py'` пуст — нигде в репо больше нет импортов из `app/`.
- `git rm Dockerfile railway.json Procfile` — `Dockerfile` (конфигуратор), `railway.json` (конфигуратор), `Procfile` (Heroku-стиль `uvicorn app.main:app` — больше не валиден).
- `git rm -f scripts/smoke_stage_6_2.py` — dead-код, опирался на `app.main`/`app.routers.main_router`/`app.routers.project_router`. Не использовался ни в CI, ни в smoke-runner'ах после UI-4 (никто на него не ссылался по grep).
- Тесты:
  - `git rm -rf tests/test_web/` — 8 файлов под `TestClient(app/main.py)`: `__init__.py`, `conftest.py`, `test_access.py`, `test_admin_budget.py`, `test_configurator_redirects.py`, `test_databases_redirects.py`, `test_healthz.py`, `test_ui1_sidebar_app.py`. Все они тестировали либо 301-редиректы с `config.quadro.tatar` (которые удалены), либо admin-страницы в app/ (которые переехали в portal).
  - `git mv tests/test_web/test_migration.py tests/test_migration.py` — обобщённый тест миграционных constraints на `db_engine`, не привязан к app/.
  - `git mv tests/test_web/test_bootstrap_admin.py tests/test_bootstrap_admin.py` — тесты `scripts/bootstrap_admin.py`. `import app.config as cfg` → `import shared.config as cfg` (3 точки замены).
  - Новый файл `tests/test_portal/test_admin.py` — 6 кейсов, заменяет `test_web/test_access.py` (3 кейса) и `test_web/test_admin_budget.py` (3 кейса). Использует `admin_portal_client`/`manager_portal_client` из `test_portal/conftest.py` — никакой отдельной admin_client_app машинерии больше не нужно.
  - `tests/conftest.py` — обновлены упоминания `app.database`/`app/database.py` в комментариях на `shared.db`/`shared/db.py`. Поведение фикстур не изменилось.
- Комментарии в коде, ссылавшиеся на `app.*` пути (актуально-misleading), обновлены на новые: `portal/routers/admin_price_uploads.py`, `portal/services/configurator/price_loader.py`, `scripts/{load_price,fix_gpu_model_names,run_matching,_dump_raw_html}.py`. Чисто-исторические упоминания (типа «раньше было через app.templating —») оставлены как контекст.
- Документация:
  - `README.md` — переписан раздел «Структура проекта» под актуальный портал (ссылка на CLAUDE.md как источник истины); `uvicorn app.main:app` → `uvicorn portal.main:app --port 8081`; `pytest tests/test_web/` → `pytest tests/test_portal/`; `app/services/price_loaders/` → `portal/services/configurator/price_loaders/`; раздел URL-ов перечислен по `/configurator/*`, `/databases/*`, `/settings/*`, `/auctions/*`, `/admin/*`. Стек: добавлены реальные поставщики (Netlab, Resurs Media, Green Place) — в старом README их не было.
  - `CLAUDE.md` — обновлены: блок «Backend» (FastAPI один сервис, не два; история переездов UI-1..UI-5 в одну строку); «Frontend» (только `portal/templates/`); «Инфраструктура» (один Railway-сервис, упоминание удаления configurator-сервисов и DNS); структура папок (убран блок `app/`, в `portal/templates/` добавлены `admin/` и `export/`); «Где что искать» (конфигуратор → `portal/routers/configurator/`; общий код используется в `portal/` и `scripts/`); блок «Сервисы конфигуратора» очищен от исторических комментариев про переезд из app/; «Расписание APScheduler» — добавлена пометка про удалённое поле `settings.run_scheduler`.
  - `docs/ui-architecture.md` — шапка переписана с «общий sidebar портала и конфигуратора» на «общий sidebar портала» (исторически устарело); раздел «Конфигуратор / app/templates/base.html» переписан под portal-only; раздел «301-редиректы со стороны конфигуратора» переписан в историческую справку; «Иконки» — убран двойной поиск в `app/_macros/icons.html`; «История» — пункт UI-5 переписан из «план» в подробное описание выполнения.
  - `docs/url-migration-map.md` — секция UI-5 переписана из «план» в полную таблицу артефактов (что удалено, что переехало, что заменено в импортах). Operational-блок: что собственнику делать руками с Railway и Reg.ru.
- pytest регрессия: **1862 passed, 1 skipped, 0 failed** (UI-4.5 baseline был 1892; разница −30 — удалена папка `tests/test_web/` с 8 файлами на 301-редиректы / healthz / sidebar в app/, и заменены 6 кейсов из `test_admin_budget.py`+`test_access.py` на 6 новых в `tests/test_portal/test_admin.py`). Нетто покрытие сохранено: admin-страницы конфигуратора тестируются из портал-контекста, миграции и bootstrap-admin переехали в `tests/test_portal/` (где есть autouse `_clean_tables`). По дороге словил два бага: (1) Write tool заменил NBSP-литерал `\xa0` в `_ru_money` на обычный пробел при копировании в `portal/templating.py` — фикс через Python-скрипт; (2) перенос `test_migration.py` и `test_bootstrap_admin.py` в корень `tests/` оставил их без autouse TRUNCATE-фикстуры — перенёс в `tests/test_portal/` ещё одним `git mv`.
...
Статус на 2026-05-11: **план закрыт полностью**. Все пять этапов Пути B выполнены:
...
- UI-4.5 — перенос `app/services/auctions/` и `app/services/catalog/` в `portal/services/`, перенос cron USD/RUB из `app/scheduler.py` в `portal/scheduler.py` (5 cron-job'ов `cbr_fetch_<HHMM>` + `ensure_initial_rate()` через portal startup); `app/scheduler.py` удалён, кросс-импорт `from app.services.{auctions,catalog} ...` устранён по всему репо (36 файлов; pytest 1892 passed);
- **UI-5 — финальная зачистка `app/` целиком.** Settings переехал в `shared/config.py`, фильтры `to_rub`/`fmt_rub` — в `portal/templating.py` (живой код, не re-export), 3 admin-страницы (`/admin`, `/admin/budget`, `/admin/queries`) — в `portal/routers/admin.py` с сохранением URL'ов. Шаблоны `kp_template.docx`/`project_template.xlsx` — в `portal/templates/export/`. 54 файла с `from app.*` обновлены на `shared.*`/`portal.*`. `app/` + `Dockerfile` + `railway.json` + `Procfile` + dead-script `smoke_stage_6_2.py` удалены. `tests/test_web/` ликвидирована (2 общеполезных файла переехали в корень `tests/`), создан новый `tests/test_portal/test_admin.py` (6 кейсов). Документация (README, CLAUDE.md, docs/ui-architecture.md, docs/url-migration-map.md) синхронизирована.

После UI-5 в проекте остался один FastAPI (`portal/main.py`), один Railway-сервис per environment, один поддомен (`app.quadro.tatar` / `app-preprod.quadro.tatar`). Папка `app/` отсутствует, `grep 'from app\.' --include='*.py' -r .` пуст. Railway-сервисы `configurator`/`configurator-preprod` и DNS-записи `config.quadro.tatar`/`config-preprod.quadro.tatar` собственник удаляет вручную после деплоя.
